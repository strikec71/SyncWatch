...
- **`src/content/invite.ts`** — инвайт-ссылки (Фаза 4): читает `#r=код&s=сервер`
  (страница `/join`) или `#syncwatch=код&s=сервер` (любой URL) → сохраняет настройки и
  шлёт `connect`. `s=` обязателен в генерируемых ссылках («Позвать» без сервера
  отказывается копировать): дефолтного публичного релея нет, без адреса приглашённому
  с чистой установкой подключаться некуда. Ошибка connect показывается красным тостом.
  Только верхний фрейм; после применения чистит хэш.
...
- **≥3 участника:** включаются **права host**; управлять могут host + те, кому host
  **выдал контроль** («Контроль»); гость просит через «Запросить». **Пауза/стоп
  ...
  ⚠️ **Гейт + авто-выравнивание (не молча!).** Заблокированное гейтом play/seek/rate уже
  исполнилось в ЛОКАЛЬНОМ плеере → просмотр участника форкается от комнаты. Хаб при этом
  (`requestGateResync` в `sync.ts`, троттлинг 3с) шлёт `MODE{detached:false}` → сервер
  идемпотентно триггерит SNAPSHOT_REQ хосту → нам прилетает направленный STATE и плеер
  откатывается к общему состоянию + тост «Управляет хост…». Раньше действие дропалось
  молча — у не-контроллера плеер уезжал, «синхрон развалился» при 3-м участнике.
